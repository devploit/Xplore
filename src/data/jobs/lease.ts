 ...
 */
export async function acquireLease(db: XploreDb, key: string, owner: string, now: number = Date.now(), ttl: number = LEASE_MS): Promise<boolean> {
...
export async function renewLease(db: XploreDb, key: string, owner: string, now: number = Date.now(), ttl: number = LEASE_MS): Promise<void> {
  const row = await db.backfill.get(key);
  if (row?.lease_owner === owner) await db.backfill.put({ ...row, lease_until: now + ttl });
}

export async function releaseLease(db: XploreDb, key: string, owner: string): Promise<void> {
  const row = await db.backfill.get(key);
