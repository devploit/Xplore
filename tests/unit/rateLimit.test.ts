...
import { backoff, decide, pageDelay, parseLimitHeaders } from "@/data/rateLimit";
...
describe("pacing", () => {
  it("waits 1 s between pages and 10 s every sixth page", () => {
    expect(pageDelay(1)).toBe(1_000);
    expect(pageDelay(5)).toBe(1_000);
    expect(pageDelay(6)).toBe(10_000);
    expect(pageDelay(12)).toBe(10_000);
  });
